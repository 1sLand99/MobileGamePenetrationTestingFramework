...
#define LUA_GLOBALSINDEX	(-10002)//这个只有5.1.5有
...
typedef struct lua_Debug {
  ...
  /* private part */
  void *i_ci;  /* active function */ //这个部分是个结构体，但没时间去弄。
}lua_Debug;
...
typedef void* (*lua_getglobalFuncType)(void *L, const char *name);
...
typedef const char *(*lua_pushstringFuncType)(void *lua_State, const char *s);
const char *(*lua_pushstring)(void *lua_State, const char *s);

typedef const char *(*lua_getupvalueFuncType)(void * lua_State, int funcindex, int n);
const char *(*lua_getupvalue)(void *L, int funcindex, int n);

typedef const char *(*lua_setupvalueFuncType)(void * lua_State, int funcindex, int n);
const char *(*lua_setupvalue)(void * lua_State, int funcindex, int n);